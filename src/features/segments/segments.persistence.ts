...
import type { SaveSegmentsResult } from './segments.api';
...
async function syncOutbox(force = false): Promise<void> {
  if (skipRemotePersistence) {
    useSegmentsStore
      .getState()
      .setPersistenceState({ persistenceStatus: 'saved', outboxSize: 0, syncError: null });
  ...
  for (const entry of outbox) {
    ...
    let result: SaveSegmentsResult;
    try {
      result = await saveCloudSegmentsSnapshot(snapshot, {
      ...
      useSegmentsStore
        ...
        .setPersistenceState({
          persistenceStatus: 'error',
          syncError: error instanceof Error ? error.message : 'Unable to reach the segments service.',
        });
      throw error;
    }
